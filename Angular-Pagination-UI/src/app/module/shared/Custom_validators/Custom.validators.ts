...
export class CustomValidator {
     ...
     * @parameter --(service: AppService)
     * @returns --({uniqueClusterNameAsyncError: true} | null)
     */
    static uniqueEmailAsync(service: AppService): AsyncValidatorFn {
        return (control: AbstractControl): Observable<ValidationErrors | null> |
            Promise<ValidationErrors | null> => {
            return service.getEmail(control.value)
                .pipe(
                    map(email => {
                        console.log('email custom validator: ', email);
                        return email ?
                            { uniqueEmailAsyncError: true } : null;
                    })
                );
        };
    }
